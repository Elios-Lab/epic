...
A digital twin is the heart of every EPIC contest. Each twin is self-contained: it owns state evolution, fault activation, and fault effects. The platform discovers twins through the plugin registry and exposes their metadata through the catalog API.
...
The registry stores sensor prototypes; the engine creates a fresh instance per session so drift, buffers, and random state never leak between contests.
...
Implement `Sensor`, declare `measured_quantity`, and register it with `sensor_registry`. If the quantity already exists in `PhysicalQuantity`, no core change is needed. New quantities belong in `epic/core/quantities.py`.
...
Implement `TaskEvaluator` and register it with `task_evaluator_registry`. A task evaluator owns payload validation, metric application, and the leaderboard ranking value for one task type.
