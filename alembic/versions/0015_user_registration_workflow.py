...
def _index_exists(table: str, index: str) -> bool:
    bind = op.get_bind()
    insp = sa.inspect(bind)
    return any(existing["name"] == index for existing in insp.get_indexes(table))
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("users") as batch_op:
        ...
        if "first_name" not in user_columns:
            batch_op.add_column(sa.Column("first_name", sa.String(128), nullable=True))
        if "last_name" not in user_columns:
            batch_op.add_column(sa.Column("last_name", sa.String(128), nullable=True))
        if "phone_number" not in user_columns:
            batch_op.add_column(sa.Column("phone_number", sa.String(32), nullable=True))

    if not _index_exists("users", "ix_users_status"):
        op.create_index("ix_users_status", "users", ["status"])

    # ── organizer_requests ────────────────────────────────────────────────
    if not _table_exists("organizer_requests"):
        op.create_table(
            "organizer_requests",
    ...
    if not _index_exists("organizer_requests", "ix_organizer_requests_email"):
        op.create_index("ix_organizer_requests_email", "organizer_requests", ["email"], unique=True)
    if not _index_exists("organizer_requests", "ix_organizer_requests_status"):
    ...
    if not _index_exists("invitations", "ix_invitations_email"):
        op.create_index("ix_invitations_email", "invitations", ["email"])
    if not _index_exists("invitations", "ix_invitations_contest_id"):
        op.create_index("ix_invitations_contest_id", "invitations", ["contest_id"])
    if not _index_exists("invitations", "ix_invitations_token"):
